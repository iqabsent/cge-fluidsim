...
// Moved from OpenALSoundController so we could also handle Audio Queues
static void MyInterruptionCallback(void* user_data, UInt32 interruption_state)
{
	BBSceneController* scene_controller = (BBSceneController*)user_data;
	OpenALSoundController* openal_sound_controller = [OpenALSoundController sharedSoundController];
	...
	if(kAudioSessionBeginInterruption == interruption_state)
	{
		...
		[scene_controller.audioQueueBackgroundMusic beginInterruption];
	}
	else if(kAudioSessionEndInterruption == interruption_state)
	{
		...
		[scene_controller.audioQueueBackgroundMusic endInterruption];
	}
}
...
@synthesize audioQueueBackgroundMusic;
...
- (id) init
{
	...
	if(nil != self)
	{
		// Moved AudioSession code to this class from OpenALSoundController so we can also handle interruptions for Audio Queues
		InitAudioSession(kAudioSessionCategory_SoloAmbientSound, MyInterruptionCallback, self, 22050.0);
...
//		[[OpenALSoundController sharedSoundController] setDistanceModel:AL_EXPONENT_DISTANCE_CLAMPED];
		...
		audioQueueBackgroundMusic = [[AudioQueueServicesController alloc] initWithSoundFile:BACKGROUND_MUSIC];
		audioQueueBackgroundMusic.gainLevel = 0.5;
	}
	return self;
}
...
-(void)addUFO
{
	...
	ufo.rotationalSpeed = BBPointMake(0.0, 0.0, 1000.0);
...
-(void) startScene
{
	...
	[audioQueueBackgroundMusic audioQueueStart]; // start playing
}
...
- (void) dealloc
{
	...
	[audioQueueBackgroundMusic release];
