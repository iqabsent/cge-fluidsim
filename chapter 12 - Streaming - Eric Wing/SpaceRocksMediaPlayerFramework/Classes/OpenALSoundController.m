...
- (id) init
{
	...
	if(nil != self)
	{
		...
		// Because we don't know what's in the iPod Library, setting the preferred hardware sample rate doesn't make much sense.
		// This function will ignore the sample rate parameter for this demo.
		InitAudioSession(kAudioSessionCategory_AmbientSound, MyInterruptionCallback, self, 44100.0);
...
- (void) initOpenAL
{
	...
	if(openALDevice != NULL)
	{
		...
		// Even though we don't set the preferred hardware sample rate, we still want to set the OpenAL mixer rate.
