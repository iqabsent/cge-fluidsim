...
- (void) applyState
{
	...
	if(YES == positionalDisabled)
	{
		...
		alSourcef(sourceID, AL_CONE_OUTER_GAIN, 0.001); // workaround for Apple bug rdar://8543491. Don't set to 0.
	...
	else
	{
		...
		// Gain at 0.0 fails when it shouldn't. New to iOS 4.1.
		if(0.0f == coneOuterGain)
		{
			// set the value small as a work around
			alSourcef(sourceID, AL_CONE_OUTER_GAIN, 0.001);
		}
		else
		{
