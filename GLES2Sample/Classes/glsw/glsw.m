...
static bstring __glsw__LoadEffectContents(glswContext* gc, bstring effectName)
{
    ...
    while (pPathList)
    {
        ...
        NSString *documentPath = [documentDir stringByAppendingPathComponent:@"Fluid.glsl"];
        const char *cDocumentPath = [documentPath cStringUsingEncoding:NSUTF8StringEncoding];
        
        NSLog(@"pth: %s",cDocumentPath);
       fp = fopen(cDocumentPath   , "rb");
        //fp = fopen((const char*) "demo/Fluid.glsl", "rb");
        if (fp)
        {
            break;
        }
